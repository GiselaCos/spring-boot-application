...
Requirements:
REQ1 - A toggle may be used by one or more application/service, and also they can be overridden, for example:
	...
	*Toggle named isButtonRed with value a true must be configured so all application/services may use the same value, with the exception for the service Abc that must have this toggle configured with the value set false.

REQ2 - When the application/service request their toggles, they must only provide their id and version.
REQ3 - Implement a Rest API to manage the toggles and to deliver the toggles values to each client application/service request.
REQ4 - When setting a new toggle, only users with admin permission may create a toggle.
REQ5 - When requesting the toggles, the client application/service must be authorised to use the Toggle Service.
REQ6 - NYI - Try to find a way to alert each client application/service that the toggle was changed.
...
*To read toggle status, your user must have role APP, username identifies the app, should call the service /readToggleAppStatus
    eg. "/readToggleAppStatus?toggleId=feature1&toggleVersion=V1.0"
*To update toggle status for a specific app, your user must have role APP, username identifies the app, should call the service /updateToggleAppStatus (toggle status has priority over toggle status for a specific ap, if toggle status is false, toggle status for a specific app is irrelevant
    eg. "/updateToggleAppStatus?toggleId=feature1&toggleVersion=V1.0&toggleStatus=true"

Running the tests

*ToggleSystemTests was created to validate the required requirements
    *eg. toggle.properties
